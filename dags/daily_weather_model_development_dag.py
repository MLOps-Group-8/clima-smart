...
from daily_compare_best_model import compare_and_select_best_model
from daily_model_validation import evaluate_and_visualize_model
from daily_model_bias_detection import calculate_metrics_for_features, bin_features
...
def train_model_task(**kwargs):
    ...
    # Define the output path for the combined models
    output_file = os.path.join(TEMP_DIR, "all_models.pkl")

    # Pass the data to the model_training function
    models_file_path = run_model_training(data_path, output_file)
...
def bias_detection_task(**kwargs):
    ...
    # Define targets and slicing features
    targets = ['apparent_temperature_max', 'sunshine_duration', 'snowfall_sum']
    slicing_features = ['temperature_2m_max_bin', 'precipitation_sum_bin']

    # Retrieve models and data paths
    models_file_path = kwargs['ti'].xcom_pull(key='models_file_path', task_ids='train_model')
    if not models_file_path:
        raise ValueError("No model file path found in XCom for 'models_file_path'.")

    # Load models and scalers
    with open(models_file_path, "rb") as f:
        models_and_scalers = pickle.load(f)

    models = {target: data["model"] for target, data in models_and_scalers.items()}
    scalers = {target: data["best_params"] for target, data in models_and_scalers.items()}

    # Perform bias detection for each target
    for target in targets:
        # Retrieve test data path
        test_data_path = kwargs['ti'].xcom_pull(key=f"{target}_X_test_path", task_ids='process_data')
        if not test_data_path:
            logging.warning(f"No test data path found for target '{target}'. Skipping bias detection.")
            continue
        ...
        X_test_df = pd.DataFrame(X_test, columns=[
            ...
            'rain_sum', 'showers_sum', 'daylight_duration', 'precipitation_sum', 
            'temperature_range', 'diurnal_temp_range', 'precipitation_intensity'
        ])

        # Add binned columns for slicing
        X_test_df['temperature_2m_max_bin'] = bin_features(
            X_test_df, 'temperature_2m_max', bins=4, labels=["cold", "cool", "warm", "hot"]
        )
        X_test_df['precipitation_sum_bin'] = bin_features(
            X_test_df, 'precipitation_sum', bins=4, labels=["low", "medium-low", "medium-high", "high"]
        )

        # Call the bias detection function
        results = calculate_metrics_for_features(
            data=X_test_df,
...
def save_models_to_gcs_task(**kwargs):
    ...
    best_model_paths = kwargs['ti'].xcom_pull(key='best_model_paths', task_ids='train_model')

    for target, model_path in best_model_paths.items():
        # Define the GCS path for the model
        gcs_path = f"assets/daily_models/{target}_best_model.pkl"
        ...
        logging.info(f"Best model for {target} saved to GCS at {gcs_path}")
