...
def evaluate_and_visualize_models(models, processed_data, targets, bucket_name, HOURLY_DATA_PLOTS_PATH='hourly_model_validation_plots/'):
    """
    Validate the models with visualizations and save plots to a GCS bucket.
    ...
    Args:
        ...
        HOURLY_DATA_PLOTS_PATH (str): Path inside GCS to save the plots.

    Returns:
        None
    """
    logging.info("Starting hourly model validation and visualization")
    ...
    for target in targets:
        ...
        scaler_target.fit(processed_data[target]["y_train"].reshape(-1, 1))  # Fit on training data
        ...
        residual_plot_path = f"{HOURLY_DATA_PLOTS_PATH}Residual_Distribution_{target}.png"
        ...
        actual_vs_predicted_path = f"{HOURLY_DATA_PLOTS_PATH}Actual_vs_Predicted_{target}.png"
        save_plot_to_gcs(plt, bucket_name, actual_vs_predicted_path)
        plt.close()

        logging.info(f"Validation plots for {target} saved to GCS.")

    logging.info("Hourly model validation and visualization completed")
